...
//FIXME - import authHeader from "./auth-header";
//FIXME - import handleError from "./error-handler";
...
const deleteCard = async (id) => {
  try {
    return (
      ...
      //FIXME -   headers: authHeader(),
      })
    ).data;
  } catch (error) {
    //FIXME - handleError(error);
    throw error;
  }
};
...
const getSpecificCard = async (idStringified) => {
  try {
    return (
      await axios.get(API_URL + "/" + idStringified, {
       headers: authHeader(),
      })
    ).data;
  } catch (error) {
...
const updateCard = async (idStringified, reqBody) => {
  ...
  try {
    return (
      await axios.put(API_URL + "/" + idStringified, reqBody, {
        headers,
      })
    ).data;
  } catch (error) {
